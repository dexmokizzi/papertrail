...
- Writes complete field definitions including qualtrics_id,
  type, mark_type, scale, page number, and regions
...
def _save(survey_name: str, field_meta: dict):
    ...
    Always writes the page number so extractor.py routes
    each field to the correct page image.

    Args:
        survey_name: Survey ID for the YAML filename.
        field_meta:  Dict mapping field_id -> metadata dict.
                     Must include 'page' key for every field.
    """
    ...
    for fid, regions in by_field.items():
        ...
        complete = {
            ...
            "page":         meta.get("page", 1),  # ← always written
            "required":     True,
            "regions":      regions,
        }
...
def _ask_page_number(image_path: str) -> int:
    ...
    The page number is used by extractor.py to route each
    field to the correct processed image. Getting this wrong
    means fields are detected against the wrong page.

    The tool infers a default from the image filename
    (e.g. _page08.jpg -> 8) so the operator can just press
    Enter if the filename matches the actual page number.
    ...
    """
    # Try to infer from filename: something_page08.jpg -> 8
    import re
    ...
    print(f"  (This must match the page number in the YAML so")
    print(f"  the pipeline reads the right image for these fields.)")
...
def _build_fields(
    ...
    Args:
        ...
        image_path:  Path to the image being calibrated.
                     Used to infer the default page number.
    ...
    # Ask page number — must happen before drawing boxes
    # so every field written to YAML has the correct page.
    ...
    # Build metadata per field — page number included
    field_meta = {}
    for fid in field_ids:
        field_meta[fid] = {
            ...
            "page":         page_number,  # ← stored per field
        }
...
def run_calibration(image_path: str, survey_name: str):
    ...
    # Interactive setup — image_path passed for page inference
